export const meta = {
  ...
  description: 'Deep research harness (angles supplied by the orchestrator) — fan-out web searches, fetch sources, adversarially verify claims, synthesize a cited report.',
  whenToUse: 'Invoked by /vault-x:research and /vault-x:grow via scriptPath. A fork of the bundled deep-research workflow that SKIPS the fragile Scope agent (which aborts runs by mis-emitting structured output) — the caller supplies the 5 search angles in args.angles. Pass args: { question, angles:[{label,query,rationale?}], caps? }.',
  phases: [{"title":"Scope","detail":"Angles supplied by orchestrator (no Scope agent)"},{"title":"Search","detail":"parallel WebSearch agents, one per angle"},{"title":"Fetch","detail":"URL-dedup, fetch top sources, extract falsifiable claims"},{"title":"Verify","detail":"3-vote adversarial verification per claim (need 2/3 refutes to kill)"},{"title":"Synthesize","detail":"Merge semantic dupes, rank by confidence, cite sources"}],
}

// deep-research-local: FORK of the bundled deep-research workflow.
// Only difference vs native: the Scope agent is removed — angles come in via
// args.angles (generated by the main loop, which emits valid structured output
// reliably). All agents pinned to opus. Everything downstream is byte-identical,
// so the return shape is unchanged. Re-sync if Anthropic updates deep-research.
// Invoke: Workflow({ scriptPath, args: { question, angles:[{label,query}], caps? } })

// args may arrive as an object OR a JSON string depending on how it's passed — normalize once.
const ARGS = (typeof args === "string")
  ? (() => { try { return JSON.parse(args) } catch { return {} } })()
  : (args || {})
const C = ARGS.caps || {}
const VOTES_PER_CLAIM = C.votes ?? 3
const REFUTATIONS_REQUIRED = C.refute ?? 2
const MAX_FETCH = C.maxFetch ?? 15
const MAX_VERIFY_CLAIMS = C.maxVerify ?? 25
...
// ─── Phase 0: Scope — angles supplied by the orchestrator (no fragile Scope agent) ───
phase("Scope")
const QUESTION = (typeof ARGS.question === "string" && ARGS.question.trim()) || ""
const ANGLES = Array.isArray(ARGS.angles) ? ARGS.angles : []
if (!QUESTION || ANGLES.length < 3) {
  return { error: "Pass args as an object: { question: '<q>', angles: [{label, query, rationale?}, … at least 3] }." }
}
const scope = { question: QUESTION, summary: "Angles supplied by the orchestrator.", angles: ANGLES }
log("Q: " + QUESTION.slice(0, 80) + (QUESTION.length > 80 ? "…" : ""))
log("Angles supplied: " + ANGLES.map(a => a.label).join(", "))
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), {
    label: "search:" + angle.label, phase: "Search", schema: SEARCH_SCHEMA, model: 'opus'
  }).then(r => {
    if (!r) return null
...
return {
  ...
  stats: {
    ...
    agentCalls: scope.angles.length + allSources.length + (voted.length * VOTES_PER_CLAIM) + 1,
  },
}
